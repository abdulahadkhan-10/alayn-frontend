...
export default function ThermalReceipt({ order, onClose }: ThermalReceiptProps) {
  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl overflow-hidden" style={{ maxHeight: "92vh" }}>

      {/* ── Premium Modal Header ── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0f1f3d] via-[#1B2A4A] to-[#1e3a5f] px-5 py-4 flex items-center justify-between shrink-0">
        {/* Decorative glow */}
        <div className="absolute -top-8 -left-8 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl pointer-events-none" />
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-black text-sm leading-none">Tax Invoice</p>
            <p className="text-gray-400 text-[11px] font-mono mt-0.5">
              Bill No: <span className="text-emerald-400 font-bold">{billNo}</span>
            </p>
          </div>
        </div>
        ...
        <div className="relative flex items-center gap-2">
          {/* Paid badge */}
          {isPaid && (
            <div className="flex items-center gap-1 px-2.5 py-1 bg-emerald-500/20 border border-emerald-400/30 rounded-lg text-emerald-300 text-[11px] font-bold">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-xl bg-white/10 hover:bg-white/20 text-gray-300 hover:text-white transition flex items-center justify-center cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      {/* ── Success Toast ── */}
      {showSuccess && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-emerald-600 text-white text-xs font-bold px-4 py-2 rounded-full shadow-xl animate-in fade-in slide-in-from-top-2 duration-300">
      ...
      {/* ── Bill Info Strip ── */}
      <div className="bg-gray-50 border-b border-gray-100 px-5 py-2.5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 text-[11px] text-gray-500 font-medium">
          <span className="flex items-center gap-1">
            {tableDisplay ? <Utensils className="w-3 h-3" /> : <CreditCard className="w-3 h-3" />}
            {orderSourceDisplay}
          </span>
          <span className="text-gray-300">|</span>
          <span>{formattedDate} at {formattedTime}</span>
        </div>
        <span className="text-[11px] font-bold text-[#1B2A4A]">
      ...
      {/* ── Footer Action Bar ── */}
      <div className="shrink-0 bg-white border-t border-gray-100 px-5 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {onClose && (
            <button
              onClick={onClose}
              className="px-4 py-2 text-xs font-bold text-gray-600 hover:text-gray-900 border border-gray-200 hover:border-gray-300 rounded-xl transition cursor-pointer"
            >
              Close
            </button>
          )}
